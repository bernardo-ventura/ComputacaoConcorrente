...
void * acharMinMax(void *arg) {
    ...
    vetorLocal[j] = minLocal; //coloca o valor minimo em um vetor na posição 0
    vetorLocal[j+1] = maxLocal; //coloca o valor máximo em um vetor na posição 1
    }
    pthread_exit((void*) vetorLocal);
}
...
int main(int argc, char *argv[]){
    ...
    for(long int i=0;i<nthreads;i++){
        if(pthread_join(*(tid+i), (void**)&retornoVetor)){
            fprintf(stderr, "erro de criação de thread");
            return 3;
        }
    }
    ...
    //     if(retornoVetor[i]<minConc){
    //         minConc = retornoVetor[i];
    //     }
    //     if(retornoVetor[i]>maxConc){
    //         maxConc = retornoVetor[i];
    //     }
    // }
    GET_TIME(finalConc);
